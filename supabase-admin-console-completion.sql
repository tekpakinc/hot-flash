...
create index if not exists vehicle_catalog_options_lookup_idx on public.vehicle_catalog_options(category,vehicle_type,parent_label,active,sort_order,label);
...
create policy "Catalog options publicly readable" on public.vehicle_catalog_options for select using(active=true or public.is_hotflash_admin());
...
begin
 ...
 select jsonb_build_object(
  ...
  'verified_members',(select count(*) from public.profiles where member_tier='verified' and verified_membership_status='active' and is_verified=true),
...
returns table(user_id uuid,email text,username text,display_name text,avatar_url text,created_at timestamptz,last_sign_in_at timestamptz,account_status text,status_reason text,vehicle_count bigint,shop_count bigint)
...
begin
 ...
 insert into public.hotflash_user_status(user_id,status,reason,updated_by,updated_at) values(p_user_id,p_status,nullif(trim(p_reason),''),auth.uid(),now())
...
returns setof public.vehicle_catalog_options language sql security definer set search_path=public as $$
 select * from public.vehicle_catalog_options where public.is_hotflash_admin() and (p_category is null or category=p_category) and (p_vehicle_type is null or vehicle_type=p_vehicle_type)
 order by category,vehicle_type,parent_label nulls first,sort_order,label; $$;

create or replace function public.admin_upsert_catalog_option(p_id uuid default null,p_category text default null,p_label text default null,p_parent_label text default null,p_vehicle_type text default 'automobile',p_sort_order integer default 0,p_active boolean default true)
...
begin
 ...
 if p_id is null then
  insert into public.vehicle_catalog_options(category,label,parent_label,vehicle_type,sort_order,active) values(p_category,trim(p_label),nullif(trim(p_parent_label),''),coalesce(nullif(trim(p_vehicle_type),''),'automobile'),coalesce(p_sort_order,0),coalesce(p_active,true)) returning id into result_id;
 else
  update public.vehicle_catalog_options set category=p_category,label=trim(p_label),parent_label=nullif(trim(p_parent_label),''),vehicle_type=coalesce(nullif(trim(p_vehicle_type),''),'automobile'),sort_order=coalesce(p_sort_order,0),active=coalesce(p_active,true),updated_at=now() where id=p_id returning id into result_id;
 end if;
 return result_id;
end; $$;

create or replace function public.admin_delete_catalog_option(p_id uuid)
returns void language plpgsql security definer set search_path=public as $$ begin if not public.is_hotflash_admin() then raise exception 'Admin access required'; end if; delete from public.vehicle_catalog_options where id=p_id; end; $$;
