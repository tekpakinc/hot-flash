document.addEventListener('DOMContentLoaded',async()=>{
  ...
  panel.addEventListener('click',async event=>{
    ...
    const result=await hotflashSupabase.rpc('cancel_vehicle_transfer',{p_transfer_id:cancel.dataset.cancelTransfer});
    if(result.error){cancel.disabled=false;setStatus(result.error.message||'Could not cancel the transfer.',true);return}
    setStatus('Transfer request cancelled.');
